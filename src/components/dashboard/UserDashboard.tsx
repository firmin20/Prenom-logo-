      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-400 mt-1">
                Prêt à créer votre prochaine identité visuelle ?
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div
              onClick={() => onTabChange('create')}
              className="rounded-2xl bg-[#0D1220] border border-white/[0.08] hover:border-amber-500/30 p-6 transition-all cursor-pointer group"
            >
          ...
          <div className="space-y-4 pt-4">
            ...
            {savedLogos.length === 0 ? (
              <div className="rounded-2xl bg-[#0B0F1B] border border-white/[0.06] p-8 text-center space-y-4">
                <p className="text-xs text-slate-400">
                  Vous n'avez pas encore sauvegardé de logo dans votre galerie.
                </p>
                <button
                  onClick={() => onTabChange('create')}
